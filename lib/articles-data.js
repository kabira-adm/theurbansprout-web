...
// text is reused verbatim. For the other 18 articles, which never had a
// short card blurb before, the text is each article's own SEO
// metadata.description (already a single, vetted sentence written for
// that article), not new copy invented for this page.
//
// No publishedDate field: checked every article page.js's metadata,
// ArticleLayout usage, and JSON-LD block (grep across all 31 files) for
// any date/publish-date field, and there isn't one anywhere in this
// codebase. The `order` field below (each article's index within its
// pillar's array) is used as a newest-first proxy instead - see
// components/ArticlesExplorer.js's SORTS.newest comment for how, and the
// PR notes for why this is a heuristic, not a real date.
export const pillars = [
  ...
  },
  {
    ...
    articles: [
      ...
      { title: "Organic pest control for kitchen garden vegetables (safe to eat)", href: "/kitchen-gardening/organic-pest-control-kitchen-garden-vegetables", description: "How to deal with aphids, whiteflies, and caterpillars on food you're growing to eat, without spraying anything you wouldn't want on your plate.", isNew: true },
    ],
  },
  {
